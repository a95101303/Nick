  ...
  */
  ...
  for (int iamp=0; iamp<1; iamp++){
    for ( int i = 17; i <= 23; i++){
      ...
      float A=h2->GetBinCenter(h2->GetMaximumBin());
      float C;
      if ((h2->GetBinCenter(bin1-1) + (h2->GetBinCenter(bin1+1)) )/2== h2->GetBinCenter(bin1))
          C=h2->GetBinCenter(bin1);
      else
          C=h2->GetBinCenter(bin1-1);
      
      float D;
      if ((h2->GetBinCenter(bin2-1) + (h2->GetBinCenter(bin2+1)) )/2== h2->GetBinCenter(bin2) )
          D=h2->GetBinCenter(bin2);
      else
          D=h2->GetBinCenter(bin2+1);
  ...
  for (int iamp=0; iamp<1; iamp++){
    ...
    for ( int i = 17; i <= 23; i++){
      ...
      int bin2 = h2->FindLastBinAbove(h2->GetMaximum()/2);
      float A=h2->GetBinCenter(h2->GetMaximumBin());
      //float C=h2->GetBinCenter(bin1-1);
      //float D=h2->GetBinCenter(bin2+1);
      
      float C;
      if ( (h2->GetMaximum()/2) - (h2->GetBinCenter(bin1)) ==0)
          C=h2->GetBinCenter(bin1-1);
      else
          C=h2->GetBinCenter(bin1);
      ...
      float D;
      if ((h2->GetMaximum()/2) - (h2->GetBinCenter(bin2)) ==0 )
          D=h2->GetBinCenter(bin2+1);
      else
          D=h2->GetBinCenter(bin2);
